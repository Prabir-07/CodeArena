...
export function ProfilePage() {
  ...
  const [editing, setEditing] = useState(false); const [notice, setNotice] = useState(''); const [error, setError] = useState('');
  if (status === 'loading') return <section className="section-wrap content-state">Loading your profile…</section>;
  if (!user) return <section className="section-wrap content-state"><h1>Your arena is waiting.</h1><p>Log in to view and update your CodeArena profile.</p><Button href="/login">Log in</Button></section>;
  const save = async (event) => { event.preventDefault(); const data = Object.fromEntries(new FormData(event.currentTarget)); setError(''); try { await updateUser(data); setEditing(false); setNotice('Profile updated successfully.'); } catch (err) { setError(err.message); } };
  return <section className="profile-page section-wrap"><div className="profile-heading"><div><p className="eyebrow">YOUR PROFILE</p><h1>{user.firstName} {user.lastName}</h1><p>@{user.username} · {user.isEmailVerified ? 'Email verified' : 'Email not verified'}</p></div><Button variant="secondary" onClick={() => logout().then(() => window.location.assign('/'))}>Log out</Button></div>{notice && <div className="form-notice">{notice}</div>}
