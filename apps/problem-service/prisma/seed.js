// Development seed for the Problem Service database.
//
// Source: apps/frontend/src/lib/api/problemService.js's PROBLEMS array — the
// only existing problem dataset anywhere in the repository. Only fields that
// are real, backend-owned problem metadata are ported over.
//
// Explicitly NOT ported (frontend/mock-only presentation data, not owned by
// Problem Service per the approved architecture):
//   - status            per-user solved/attempted/todo — requires real
//                        submission history (Judge Service territory).
//   - acceptanceRate    submission-derived aggregate, not intrinsic problem data.
//   - solvedCount       submission-derived aggregate.
//   - submissionCount   submission-derived aggregate.
//
// TestCase rows are intentionally NOT seeded. The mock dataset has no hidden
// test cases at all (its "judging" is randomized, not case-based) — there is
// nothing genuine to port, and inventing them would violate the instruction
// not to fabricate test data. Seeded problems legitimately have zero test
// cases until an admin authors them through the future authoring API.
// Local generated client, not the bare `@prisma/client` package — see the
// `output` path in schema.prisma and the matching import in config/prisma.js.
...
// Ported verbatim from apps/frontend/src/lib/api/problemService.js.
const PROBLEMS = [
  {
    slug: 'two-sum',
    title: 'Two Sum',
...
// The mock source has only bare param names — no per-param types and no
// return type. Rather than inventing types that aren't in the source data,
// they are seeded as `null` in the approved { name, params, returnType }
// shape and left for an admin to fill in later through the authoring API.
function toFunctionSignature(signature) {
  return {
    name: signature.name,
    params: signature.params.map((name) => ({ name, type: null })),
    returnType: null,
  };
}

async function seedProblem(problem) {
  const { slug, title, difficulty, tags, summary, description, examples, constraints, signature } = problem;
  ...
  const record = await prisma.problem.upsert({
    ...
    update: {
      ...
      description,
      constraints,
      functionSignature: toFunctionSignature(signature),
    },
    create: {
      slug,
      title,
  ...
  // Examples are fully replaced on every run — the mock source has no stable
  // per-example id to upsert against, and full-replace is what keeps re-runs
  // idempotent without duplicating rows.
  await prisma.example.deleteMany({ where: { problemId: record.id } });
  ...
  for (const tagName of tags) {
    ...
    await prisma.problemTag.upsert({
      where: { problemId_tagId: { problemId: record.id, tagId: tag.id } },
      update: {},
      create: { problemId: record.id, tagId: tag.id },
    });
  }
}
...
async function main() {
  ...
  const [problemCount, tagCount, exampleCount, testCaseCount] = await Promise.all([
  ...
  console.log(`Seeded ${problemCount} problems, ${tagCount} tags, ${exampleCount} examples.`);
  console.log(`TestCase rows: ${testCaseCount} (none seeded — hidden test cases are authored later, not fabricated from mock data).`);
}
